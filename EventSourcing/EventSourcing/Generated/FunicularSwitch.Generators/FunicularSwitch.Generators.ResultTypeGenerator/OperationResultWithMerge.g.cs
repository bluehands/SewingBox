...
#pragma warning disable 1591
    ...
    public static partial class OperationResultExtension
    {
        ...
        public static OperationResult<T> FirstOk<T>(this IEnumerable<OperationResult<T>> results, Func<Failure> onEmpty)
        {
        ...
        public static OperationResult<IReadOnlyCollection<T>> AllOk<T>(this IEnumerable<T> candidates, Func<T, IEnumerable<Failure>> validate) =>
            candidates
        ...
        public static OperationResult<IReadOnlyCollection<T>> AllOk<T>(this IEnumerable<OperationResult<T>> candidates,
            Func<T, IEnumerable<Failure>> validate) =>
            candidates
                .Bind(items => items.AllOk(validate));

        public static OperationResult<T> Validate<T>(this OperationResult<T> item, Func<T, IEnumerable<Failure>> validate) => item.Bind(i => i.Validate(validate));

        public static OperationResult<T> Validate<T>(this T item, Func<T, IEnumerable<Failure>> validate)
        {
            var errors = validate(item).ToList();
            return errors.Count > 0 ? OperationResult.Error<T>(MergeErrors(errors)) : item;
        }

        public static OperationResult<T> FirstOk<T>(this IEnumerable<T> candidates, Func<T, IEnumerable<Failure>> validate, Func<Failure> onEmpty) =>
            candidates
        ...
        static Failure MergeErrors(IEnumerable<Failure> errors)
        {
            var first = true;
            Failure aggregated = default!;
        ...
        static Failure MergeErrors(Failure aggregated, Failure error) => aggregated.Merge(error);

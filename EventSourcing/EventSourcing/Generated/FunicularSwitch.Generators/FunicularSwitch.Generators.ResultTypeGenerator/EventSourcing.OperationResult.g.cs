...
#pragma warning disable 1591
    ...
    {
        public static OperationResult<T> Error<T>(Failure details) => new OperationResult<T>.Error_(details);
        ...
        public static OperationResult<T> Try<T>(Func<T> action, Func<Exception, Failure> formatError)
        {
            try
            {
                return action();
        ...
        public static async Task<OperationResult<T>> Try<T>(Func<Task<T>> action, Func<Exception, Failure> formatError)
        {
            try
            {
                return await action();
    ...
    {
        public static OperationResult<T> Error(Failure message) => Error<T>(message);
        ...
        public void Match(Action<T> ok, Action<Failure>? error = null) => Match(
            v =>
            {
                ok.Invoke(v);
                return 42;
        ...
        public T1 Match<T1>(Func<T, T1> ok, Func<Failure, T1> error)
        {
            return this switch
            {
        ...
        public async Task<T1> Match<T1>(Func<T, Task<T1>> ok, Func<Failure, Task<T1>> error)
        {
            return this switch
            {
        ...
        public Task<T1> Match<T1>(Func<T, Task<T1>> ok, Func<Failure, T1> error) =>
        ...
        public T Match(Func<Failure, T> error) => Match(v => v, error);
...
#pragma warning restore CS0168 // Variable is declared but never used
    ...
    public static partial class OperationResultExtension
    {
        ...
        public static OperationResult<T> MapError<T>(this OperationResult<T> result, Func<Failure, Failure> mapError) =>
        ...
        public static OperationResult<T1> As<T, T1>(this OperationResult<T> result, Func<Failure> errorTIsNotT1) =>
            result.Bind(r =>
            {
...
namespace EventSourcing.Extensions
{
    public static partial class OperationResultExtension
    {
        ...
        public static OperationResult<T> As<T>(this object item, Func<Failure> error) =>
            !(item is T t) ? OperationResult.Error<T>(error()) : t;

        public static OperationResult<T> NotNull<T>(this T? item, Func<Failure> error) =>
            item ?? OperationResult.Error<T>(error());

        public static OperationResult<string> NotNullOrEmpty(this string? s, Func<Failure> error)
            => string.IsNullOrEmpty(s) ? OperationResult.Error<string>(error()) : s!;

        public static OperationResult<string> NotNullOrWhiteSpace(this string? s, Func<Failure> error)
            => string.IsNullOrWhiteSpace(s) ? OperationResult.Error<string>(error()) : s!;

        public static OperationResult<T> First<T>(this IEnumerable<T> candidates, Func<T, bool> predicate, Func<Failure> noMatch) =>
            candidates
